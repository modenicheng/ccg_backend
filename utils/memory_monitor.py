...
class MemoryMonitor:
    ...
    def _format_memory_report(self, memory_info: dict[str, Any]) -> str:
        ...
        lines.append(f"  已使用: {memory_info['system_used_mb']:.2f} MB ({memory_info['system_percent']:.1f}%)")
    ...
    try:
        while True:
            ...
            if detailed:
                ...
                logger.info(f"内存使用: RSS={rss_mb:.2f}MB, VMS={vms_mb:.2f}MB, 系统={system_percent:.1f}%")
