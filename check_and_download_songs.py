...
async def check_and_download_songs(room_id: str):
    ...
    async with httpx.AsyncClient() as client:
        ...
        for i, room_song in enumerate(songs, 1):
            ...
            print(
                f"  {i}. {title} - {artist} (Song ID: {song_id}, Platform ID: {platform_song_id})"
            )
        ...
        print("\nTips:")
        print("  - Download tasks run in background, may take some time")
...
if __name__ == "__main__":
    ...
    input_room_id = sys.argv[1]
    print(f"Room ID: {input_room_id}")
    print("=" * 60)

    asyncio.run(check_and_download_songs(input_room_id))
