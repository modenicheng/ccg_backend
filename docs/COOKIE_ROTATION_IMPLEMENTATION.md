...
- ✅ **单 Cookie 和多 Cookie 配置管理**（单一 → 列表 → 数据库，即使单 Cookie 也使用轮换系统）
- ✅ 轮流轮换策略（Round-Robin）
- ✅ 故障自动转移（失败3次标记失效 → 轮换）
...
即使只配置一个 Cookie，系统也会启用轮换管理器，好处包括：

- **一致性**：相同的错误处理逻辑
- **可测试性**：无需多个 Cookie 即可验证轮换逻辑
- **可升级性**：从单 Cookie 升级到多 Cookie 时无需改代码
...
API 调用失败
  ↓
标记 Cookie 失效 (failed_count++)
  ↓
失败3次?
  ├─ 是 → 标记为不健康 (is_healthy = False)
  │       轮换回自己（因为只有一个）
  │       触发异步刷新以恢复
  │       记录审计日志（FROM: A, TO: A, reason: "failure"）
  │
  └─ 否 → 返回错误
```
...
- **单 Cookie 配置**（✨ v2新增）：即使仅配置一个 Cookie，也会通过轮换系统管理，保证一致的错误处理和恢复能力
- **旧的单 Cookie 方式**：`CCG_QQ_MUSIC_COOKIE` 环境变量仍完全支持
- 无配置 Cookies 时：会降级到无凭证会话（当前行为）
...
2. 查看应用日志中的轮换日志：应该看到 `Rotated cookie to xxx (reason: keyerror)` 或 `Rotated cookie to xxx (reason: api_error)`
3. 对于多 Cookie 场景：确认已配置 2+ 个 Cookies
4. 对于单 Cookie 场景（✨升级功能）：轮换会导致回到自己，但会标记失效并触发异步刷新恢复
5. 查看 `cookie_rotation_logs` 表验证轮换是否被记录

### 单 Cookie 与多 Cookie 的行为差异

```
场景                  | 失败处理                   | 轮换结果
----------------------|--------------------------|------------------
多 Cookie (3+)        | 失败3次 → 标记不健康 → 轮换 | 切换到下一个 Cookie
单 Cookie              | 失败3次 → 标记不健康 → 轮换 | 回到自己 + 触发刷新恢复
Custom cookie_str     | 使用自定义Cookie，无轮换  | （不使用轮换系统）
```

**推荐用法**：

- 仅配置一个 Cookie 时，系统会自动通过轮换系统管理（保证一致性和恢复能力）
- 若要使用自定义 Cookie 字符串，请直接调用 API（不使用轮换）
