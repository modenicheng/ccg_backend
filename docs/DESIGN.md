...
- **抢答机制**：玩家可随时点击抢答，后端维护一个抢答队列（Redis Sorted Set，score = `server_ts * 0.0001 + offset_ts`，以 `offset_ts` 为主序、`server_ts` 为微调，保证同一毫秒内的顺序唯一性）。同时维护一个 `answer_queue_player_index`（Redis Hash，`player_id → member_json`），用于 O(1) 重复检测与移除。入队操作使用原子写入（`HSETNX` + `ZADD`），`HSETNX` 先占位防止并发重复，失败时自动回滚索引。当有玩家抢答时，若播放中则立即暂停播放（广播 `PAUSE` 事件），该玩家进入作答轮次；若已暂停，则新抢答者仅入队等待。在此期间其他玩家仍可继续抢答，但会被加入队列，待当前玩家作答完毕后再依次处理。
...
5. **抢答排队**：
   - 玩家发送 `ATTEMPT_ANSWER` 事件，后端通过原子操作（`HSETNX` 占位 + `ZADD` 入队）将玩家加入 Redis 抢答队列（Sorted Set），防止并发重复抢答。
   - 若已有当前作答玩家，新抢答者仅入队等待，不抢占当前作答权。
   - 若当前无人作答（包括播放中和已暂停两种情况）：若播放中则立即暂停（广播 `PAUSE` 事件），从队列中选出下一位作答者，设置当前作答玩家，广播 `YOUR_TURN`。
6. **作答轮次**：
...
- **抢答队列** `room:{roomId}:answer_queue` (Sorted Set)
  - 成员为 `AnswerQueueItem` JSON 字符串，score = `server_ts * 0.0001 + offset_ts`
  - 以 `offset_ts`（玩家本地播放进度）为主序，`server_ts` 为微调，保证同一毫秒内多玩家的顺序唯一性
  - 通过 `ZRANGE` 按 score 升序取出，即为抢答先后顺序
- **抢答队列玩家索引** `room:{roomId}:answer_queue:player_index` (Hash)
  - 字段为 `player_id`，值为对应的 `AnswerQueueItem` JSON
  - 用于 O(1) 重复检测（`HSETNX` 原子占位）和 O(1) 移除（通过索引获取成员后 `ZREM`），避免遍历 Sorted Set + JSON 反序列化
- **抢答队列尾部边界** `room:{roomId}:answer_queue:tail_player` (String)
  - 记录上一轮处理到的玩家 ID，用于确定下一位作答者（从该边界之后的队首取人）
...
- **`START_POS_UPDATE`** (C→S / S→C)
  ...
  ```
  - 房主调整时发送 (C→S)，后端校验范围 0-100 后广播给所有客户端 (S→C)
- **`ATTEMPT_ANSWER`** (C→S)
  ```json
  {
    "progress_ms": 12345
