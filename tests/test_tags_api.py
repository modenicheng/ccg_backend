...
async def test_patch_tag_group_basic(client: TestClient, session: AsyncSession):
    ...
    response = client.patch(
        f"/api/tags/groups/",
        json={"id": tag_group.id, "name": "Updated Group"}
    )
...
@pytest.mark.asyncio
async def test_patch_tag_group_description_null(client: TestClient, session: AsyncSession):
    ...
    response = client.patch(
        f"/api/tags/groups/",
        json={"id": tag_group.id, "description": None}
    )
    assert response.status_code == 200
    data = response.json()
...
async def test_patch_tag_group_add_tags(client: TestClient, session: AsyncSession):
    ...
    response = client.patch(
        ...
        json={
            "id": tag_group.id,
            "add_tags": [{"name": "New Tag 1"}, {"name": "New Tag 2"}],
...
@pytest.mark.asyncio
async def test_patch_tag_group_add_and_remove(client: TestClient, session: AsyncSession):
...
@pytest.mark.asyncio
async def test_patch_tag_group_invalid_existing_tag(client: TestClient, session: AsyncSession):
